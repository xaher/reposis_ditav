    ...
    <xsl:choose>
      ...
      <xsl:when test="mods:titleInfo/mods:title">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:choose>
              <xsl:when test="mods:titleInfo[not(@type='uniform' or @type='abbreviated' or @type='alternative' or @type='translated')]/mods:title">
              ...
              <xsl:otherwise>
                <xsl:apply-templates select="mods:titleInfo[@type='translated'][1]" mode="mods.printTitle">
                  <xsl:with-param name="asHTML" select="$asHTML" />
                  <xsl:with-param name="withSubtitle" select="$withSubtitle" />
                </xsl:apply-templates>
              </xsl:otherwise>
