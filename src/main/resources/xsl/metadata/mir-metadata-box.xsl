...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink"
  ...
  <xsl:template match="/">
       ...
       -->
        <table class="mir-metadata">
            ...
            <xsl:apply-templates mode="present"
                                 select="mycoreobject/metadata/def.modsContainer/modsContainer/mods:mods/mods:classification[not(@displayLabel='lod_archives')]"/>
            <xsl:apply-templates mode="present"
            ...
            <xsl:apply-templates mode="present"
                       select="mycoreobject/metadata/def.modsContainer/modsContainer/mods:mods/mods:classification[@displayLabel='lod_archives']"/>
            <xsl:call-template name="printMetaDate.mods">
                <xsl:with-param name="nodes"
    ...
    <xsl:template match="mods:name" mode="displaySubject">
        <xsl:call-template name="authorityLink">
            <xsl:with-param name="content">
                <xsl:value-of select="mods:displayForm | mods:namePart[not(@type)]"/>
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>

    <xsl:template match="mods:classification" mode="displaySubject">
        <xsl:call-template name="authorityLink">
            <xsl:with-param name="content">
                <xsl:value-of select="."/>
            </xsl:with-param>
